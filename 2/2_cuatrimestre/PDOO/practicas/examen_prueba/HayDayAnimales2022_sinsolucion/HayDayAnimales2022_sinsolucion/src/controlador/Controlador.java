...
public class Controlador {
    ...
    public boolean producir(Animal animal, TipoProductoAnimal tipo){
        ...
        if (exito){ //¿Puede producir?
                ...
                int duracion = 0;
                // TODO Segun tipo de animal, restar pienso y obtener la duracion
                // ****
                
                switch(animal.getTipo()){
                    case VACA: 
                        piensoVacaDisponible -= animal.getPienso();
                        duracion = Leche.DURACION;
                        break;
                    default:
                        piensoGallinaDisponible -= animal.getPienso();
    ...
    private void recolectar(Animal animal){
        ...
        for(ProductoAnimal pa : recolecta){
            //TODO aumentar experiencia
            experiencia += pa.getExperienciaProduccion();
            
           //TODO //Actualizar productos disponibles tras la recolecta
           productos.add(pa);
        }
        subirNivel();
    ...
    public boolean vender(TipoProductoAnimal tipo){
        ...
        if(exito){
            //cobrar segun el tipo de producto
            switch(tipo){
                case HUEVO:  
                    dinero += Huevo.PRECIO_VENTA;
                    break;
                case LECHE:  
                    dinero += Leche.PRECIO_VENTA;
                    break;
            }
    ...
    private ProductoAnimal getProductoDisponible(TipoProductoAnimal tipo){
        ...
        for (ProductoAnimal pa : productos){
            if (pa.getTipo() == tipo){
                producto = pa;
                break;
            }
    ...
    public void alimentarVacas(){
        piensoVacaDisponible -= totalVacas()*Vaca.PIENSO;
        if (piensoVacaDisponible < 0)
            piensoVacaDisponible = 0;
        
        vista.actualizarVista();;
        vista.mostrarVista();
    }
    
    public int totalVacas(){
        int total = 0;
        for (Animal a : animales){
            if (a.getTipo() == TipoAnimal.VACA)
                total++;
        }
        return total;
    }
