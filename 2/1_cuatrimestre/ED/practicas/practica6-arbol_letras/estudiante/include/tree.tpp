...
tree<T>::preorder_iterator::operator++() {
    ...
    if(!current.wrap->l_child.is_null()){
        current = current.wrap->l_child;
        level++;
    }else if(!current.wrap->r_sibling.is_null()){
        current = current.wrap->r_sibling;
    }else {
        found = false;
        while(!current.wrap->father.is_null() && !found){
            if(!current.wrap->father.wrap->r_sibling.is_null()){
                current = current.wrap->father.wrap->r_sibling;
                level--;
                found = true;
            }else {
                current = current.wrap->father;
                level--;
            }
        }
    }
...
tree<T>::cbegin_preorder() const {
    const_preorder_iterator it(root);
//    return ++it;
    return it;
}
...
tree<T>::cend_preorder() const {
    return const_preorder_iterator(node());
}