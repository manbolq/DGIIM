...
int main(int argc, char** argv) {
    ...
    computeActivity(filteredES,activityWeek);
    ...
    int traffic[MAXEVENT] = {0};
    
    if(reportby == "")
        reportby = "Brand";

    findUnique(filteredES, reportby, fields, n);
    
    cout << filteredES.to_string() << endl;
    for(int i=0; i < n; i++)
        cout << fields[i];
    
    double price[MAXEVENT] = {0.0};
    
    for(int i = 0; i < n; i++){
        price[i] = sumPrice(rawFilterEvents(filteredES, reportby, fields[i]));
        traffic[i] = rawFilterEvents(filteredES, reportby, fields[i]).size();
    }
    ...
    EventSet subset(rawFilterEvents(filteredES, reportby, fields[posMaxTraffic]));
    
    Index trafficIndx;
    trafficIndx.build(subset, onBrand);
    ...
    cout << "Highest traffic choice (" << fields[posMaxTraffic] << ") with " << maxTraffic << " hits.";
