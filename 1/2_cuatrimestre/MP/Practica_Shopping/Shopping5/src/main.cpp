...
Matrix PageRank(const Matrix & M, int max_iter, double threshold, int n){
    ...
    int cols = M.columns();
    string * brands;
    brands = new string [cols];
    for (int i = 0; i < cols; i++)
        brands[i] = M.labelAtCol(i);
    
    string name[1] = {"pr"};
    Matrix Pr(name, 1, brands, cols, 1.0/cols);
    
    double * stabilityValues = nullptr;
    stabilityValues = new double [n];
    ...
    int i = 0;
    double sum = 0; 
    for (i; i < max_iter && !conv; i++){
        sum = 0;
        Matrix aux = Pr;
        cout << endl;
        cout << "PageRank at iteration " << i << endl;
        cout << Pr << endl;
        //Pr = Pr * M;
        aux = aux * M;
        ...
        for (int j = 0; j < cols; j++){
            Pr = aux;
            sum+= abs(Pr(0, j) - aux(0, j));
        }
        conv = sum < threshold;
        
        stabilityValues[i] = sum;
    }
    
    cout << endl << "Convergence reached in " << i++ << " iterations (stability last value = " << sum << ")" << endl;
    
    cout << endl << "First " << n << " stability values" << endl;
    for (int i = 0; i < n; i++){
        cout << "    " << stabilityValues[i];
    }

    brands = nullptr;
    delete [] brands;
    
    stabilityValues = nullptr;
    delete [] stabilityValues;
...
Matrix computePageRank(Matrix & M, int max_iter, double threshold, int n = 10) {
    ...
    for (int i = 0; i < M.rows(); i++){
        for (int j = 0; j < M.columns(); j++){
            M(i, j) = alfa * M(i, j) + (1.0 - alfa)/M.columns();
        }
    }
...
void createAdjacencyMatrix(const EventSet &evs, const Index & idx, Matrix & m_adj) {
    ...
    for (int i = 0; i < idx.size(); i++){
        ...
        for (int j = lb; j < ub; j++){
            ...
            if ( brand1 !=  brand2 && evs.at(idx.at(i).getPos()).getSession() == evs.at(idx.at(j).getPos()).getSession()) 
                m_adj(brand1, brand2)++;
        }
    }
}
