...
int main(int argc, char** argv) {
    ...
    bool noargs = false;
    /*
    //para leer el objeto de la clase event, leer una linea y asignarselo al event
    e.read(ifile);*/
    ...
    correct = originalES.read(ifile, n2Read);
    ...
    string differentFields[filteredES.size()];
    int numberOfField = 0;
    
    if (reportby == "") 
        reportby = "Brand";
    
    findUnique(filteredES, reportby, differentFields, numberOfField);
    
    cout << filteredES.to_string() << endl;
    
    for (int i=0; i<numberOfField; i++) 
        cout << differentFields[i];
    
    double moneySpent[numberOfField] = {0};
    int traffic[numberOfField] = {0};
    
    for (int i=0; i<numberOfField; i++) {
        moneySpent[i] = sumPrice(rawFilterEvents(filteredES, reportby, differentFields[i]));
        traffic[i] = rawFilterEvents(filteredES, reportby, differentFields[i]).size();
    }
    
    DataVector amountData;
    amountData.alloc(numberOfField);
    amountData.setTitle("Money/" + reportby);
    amountData.loadLabels(differentFields);
    amountData.loadValues(moneySpent);
    
    DataVector hitsData;
    hitsData.alloc(numberOfField);
    hitsData.setTitle("Traffic/" + reportby);
    hitsData.loadLabels(differentFields);
    hitsData.loadValues(traffic);
    
    int maxTraffic = traffic[0];
    int max_pos = 0;
    
    for (int i=1; i<numberOfField; i++) {
        if(traffic[i]>maxTraffic) {
            maxTraffic=traffic[i];
            max_pos = i;
        }
    }
    
    EventSet mostTrafficSet(rawFilterEvents(filteredES, reportby, differentFields[max_pos]));
    
    Index index_traffic;
    
    index_traffic.build(mostTrafficSet, onBrand);
    ...
    cout << "Highest traffic choice (" << differentFields[max_pos] << ") with " << maxTraffic << " hits.";
...
void computeActivity(EventSet evSet, int arrayA[]) {
    ...
    //arrayA contiene los dias de la semana y cuántos eventos hay en cada uno
...
EventSet extractEventSet(EventSet es, Index indx) {
    ...
    for (int i=0; i<indx.size(); i++) {
        nuevo.add(es.at(indx.at(i).getPos()));   
    }
    return nuevo;
}
